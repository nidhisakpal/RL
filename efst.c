...
struct gst_proplist *	plist;
	...
	/* PSW DEBUG: Check if battery values are preserved in pts */
	fprintf(stderr, "DEBUG: After _gst_create_pset, checking first 3 points:\n");
	for (int debug_i = 0; debug_i < nterms && debug_i < 3; debug_i++) {
		fprintf(stderr, "DEBUG POINT %d: x=%.6f y=%.6f battery=%.6f\n",
		        debug_i, pts->a[debug_i].x, pts->a[debug_i].y, pts->a[debug_i].battery);
	}
	...
	/* PSW: Compute global normalization constants after FSTs are loaded */
	_gst_compute_global_normalization (cip);
	...
	for (i = 0; i < einfo.ntrees; i++) {
		...
		// cip->battery_score[i] = fsp->battery_score; /*PSW*/
		count += k;
	}
	ip1 = NEWA (count, int);
	for (i = 0; i < einfo.ntrees; i++) {
...
found_efst:
	...
	if (rp NE NULL) {
		...
		/* Standard pruning: only keep if shorter (removed battery-aware pruning) */
		if (length >= fsp -> tree_len) {
			/* New FST is not shorter, discard it */
			return (fsp -> tree_len);
		}
		/* Keep the new one!  Delete the old one. */
		*hookp = rp -> next;
		rp2 = rp -> forw;
		rp1 = rp -> back;
		rp2 -> back = rp1;
	...
	if (size <= 2) {
		...
		fprintf(stderr, "DEBUG: 2-terminal FST - tp[0].battery=%.2f, tp[1].battery=%.2f\n",
		        tp[0].battery, tp[1].battery);
		...
		i= eip -> termindex [0];
		tp [0]	    = eip -> pts -> a [i];
		fprintf(stderr, "DEBUG: Multi-terminal FST - tp[0] from pts[%d].battery=%.2f\n",
		        i, tp[0].battery);
		new_tlist [0]	= i;
		sp	    = new_steiners -> a;
		ep	    = edges;
		...
		sp -> battery = 0.0;  /* PSW: Steiner points have no battery */
		ep -> p1    = termidx++;
		ep -> p2    = previdx;
	...
	fsp -> nedges		= nedges;
	fsp -> edges		= edges;
	fsp->battery_score = 0.0;
	for (i = 0; i < fsp->terminals->n; i++) {
		int idx = fsp->tlist[i];   // already 0-based terminal index
		double bval = eip->pts->a[idx].battery;
		fsp->battery_score += bval;
		fprintf(stderr, "DEBUG EFST Multi: Terminal %d (idx %d): battery=%.2f, running_total=%.2f\n",
				i, idx, bval, fsp->battery_score);
	}
	fprintf(stderr, "DEBUG EFST Multi: Final battery_score=%.2f\n", fsp->battery_score);
...
struct point		nnsp;
	...
	if (eqpk->R EQ NULL) {
		/* This is a terminal */
		t = eqpk->origin_term;  /* safer than eqpk - eip->eqp */
		if (t < 0 || t >= eip->pts->n) {
			fprintf(stderr,
					"WARNING in build_efst_graph: origin_term=%d out of range (pts->n=%d)\n",
					t, eip->pts->n);
...
struct pset *		terms;
	...
	for (rp1 = rp2 -> forw; rp1 NE rp2; rp1 = rp1 -> forw) {
		...
		for (i = 0; i < terms->n; i++) {
			...
			if (j < 0 || j >= from_n) {
				fprintf(stderr,
						"ERROR in renumber_terminals: tlist[%d]=%d out of range (from_n=%d)\n",
						i, j, from_n);
				continue;  /* skip instead of abort */
			}

			j = rev_map[j];
			if (j < 0 || j >= to_n) {
				fprintf(stderr,
						"ERROR in renumber_terminals: rev_map gives %d out of range (to_n=%d)\n",
						j, to_n);
				continue;
			}
...
int *			tlist;
	...
	for (i = 0; i < ndg; i++) {
		...
		while (ip1 < ip2) {
			...
			fsp -> tree_len	 = 0.0;
			fsp -> battery_score = eip->pts->a[t].battery +
                       eip->pts->a[u].battery;  /* use actual battery levels */
			fprintf(stderr, "DEBUG EFST 2-term: t=%d battery=%.2f, u=%d battery=%.2f, total=%.2f\n",
					t, eip->pts->a[t].battery, u, eip->pts->a[u].battery, fsp->battery_score);
			fsp -> tlist	 = tlist;
			fsp -> terminals = terms;
			fsp -> steiners	 = NULL;
			fsp -> nedges	 = 1;
