...
static char *		me;
static char * flag_battery_file = NULL; /*PSW: global pointer for battery levels*/
...
struct PRNG_options	options;
	...
	{
		double *x = malloc(flag_num_points * sizeof(double));
		double *y = malloc(flag_num_points * sizeof(double));
		double *battery = malloc(flag_num_points * sizeof(double));

		FILE *bf = NULL;
		if (flag_battery_file != NULL) {
			bf = fopen(flag_battery_file, "r");
			if (!bf) { perror("battery_file"); exit(1); }
		}

		obj->gen_points_array(obj, x, y, flag_num_points);

		for (int i = 0; i < flag_num_points; i++) {
			if (bf) {
				if (fscanf(bf, "%lf", &battery[i]) != 1) {
					fprintf(stderr, "Not enough values in %s\n", flag_battery_file);
					exit(1);
				}
			} else {
				battery[i] = (rand() % 1000) / 10.0;
			}
...
static char *	arg_doc [] = {
	...
	"\t\tare generated.",
	"\t-s path\tIf given path exists, read initial seed from", /*PSW: */ 
    "\t-B path\tOptional file with battery levels, one per line.", /*PSW: */
	NULL
};
