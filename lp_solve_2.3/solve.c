                      ...
                      REAL *pcol)
{
  ...
  /* PSW: Add iteration logging for debugging convergence issues */
  if (lp->iter % 1000 == 0 || lp->iter <= 10) {
    fprintf(stderr, "ITER %d: row=%d, varin=%d, theta=%.6f, up=%.6f\n",
            lp->iter, row_nr, varin, *theta, up);
  }
...
static int solvelp(lprec *lp)
{
  ...
  for(;;) {
    ...
    if(feasible) {
      fprintf(stderr, "LP PHASE: Starting primal phase (iter=%d)\n", lp->iter);
      primloop(lp);
    }
    else {
      fprintf(stderr, "LP PHASE: Starting dual phase (iter=%d)\n", lp->iter);
      dualloop(lp);
      if(Status == SWITCH_TO_PRIMAL) {
        fprintf(stderr, "LP PHASE: Switching to primal (iter=%d)\n", lp->iter);
	primloop(lp);
      }
    }
		     ...
		     int     recursive)
{
  ...
  if(failure == OPTIMAL) { /* there is a good solution */
    ...
    /* CUSTOM MIP GAP CHECK: Stop if gap is small enough even with fractional solution */
    if(notint && lp->best_solution[0] != lp->infinite) {
      REAL mip_gap;
      REAL mip_gap_tolerance = lp->epsilon * 100.0; /* Convert epsilon to percentage */
      if(lp->maximise)
        mip_gap = (lp->best_solution[0] - lp->solution[0]) / my_abs(lp->best_solution[0]);
      else
        mip_gap = (lp->solution[0] - lp->best_solution[0]) / my_abs(lp->best_solution[0]);

      if(mip_gap <= mip_gap_tolerance) {
        if(lp->verbose)
          printf("%% MIP gap %.6f%% <= tolerance %.6f%%, accepting current best integer solution\n",
                  mip_gap * 100.0, mip_gap_tolerance * 100.0);
        Level--;
        return(OPTIMAL);
      }
    }
