		...
		for (;;) {
			if (vp1 >= vp2) {
				...
				*ep1++ = e;
				...
				/* PSW: Use multi-objective edge cost if in budget mode */
				double edge_cost = (double) (cip -> cost [e]);
				...
				if (multi_obj_env != NULL) {
					...
					/* Calculate normalized costs for current edge */
					double tree_cost = (double) (cip -> cost [e]);
					double battery_cost_sum = 0.0;

					/* Recalculate from edge terminals */
					if (cip -> pts != NULL) {
						int j, k;
						int nedge_terminals = cip -> edge_size[e];
						int *edge_terminals = cip -> edge[e];
					...
					edge_cost = scaled_tree_cost + battery_cost_term;
				}
