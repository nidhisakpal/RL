...
static int generate_points_array(struct PRNG_obj *obj,
                                 double *x, double *y,
                                 int npoints);
...
struct my_data *	mdp;
	...
	p -> gen_points_array = generate_points_array; /*PSW*/
...

/* PSW:
 * Generate points into separate x[] and y[] arrays.
 */
static int generate_points_array(struct PRNG_obj *obj,
                                 double *x, double *y,
                                 int npoints)
{
    ...
    FATAL_ERROR_IF ((obj EQ NULL) OR (x EQ NULL) OR (y EQ NULL));

    mdp = (struct my_data *)(obj->opaque);

    // Use the existing gen_array, but it produces interleaved x,y pairs
    double *tmp = malloc(2 * npoints * sizeof(double));
    if (tmp == NULL) return (ENOMEM);

    int rc = obj->gen_array(obj, tmp, npoints);
    if (rc != 0) {
        free(tmp);
        return rc;
    }
