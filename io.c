...
double			**points,	/* OUT - list of coordinates (+ battery) */
...
coord_t			x;
coord_t			y;
double			b;   /* PSW: battery */
struct numlist *	list;
struct numlist *	nlp;

	GST_PRELUDE

	/* Read 3 numbers per point (x, y, battery) and output 3 (x, y, battery). */
	int input_numbers_per_point = 3;   /* PSW: input format */
	int output_numbers_per_point = 3;  /* PSW: output format - now includes battery */

	/* Gather all numbers from the input in an accurate scaled form. */
	list = parse_input_numbers (fp, input_numbers_per_point * maxpoints);
	...
	/* Count how many numbers we got in total. */
	n = 0;
	...
	/* Input must be divisible by input_numbers_per_point (3). */
	if ((n % input_numbers_per_point) NE 0) {
		(void) fprintf (stderr,
		                "Input format error: expected %d numbers per point, got %d.\n",
		                input_numbers_per_point, n);
		exit (1);
	}
	n /= input_numbers_per_point;

	if (points NE NULL) {
		if (maxpoints EQ 0) {
			*points = NEWA (output_numbers_per_point * n, double);

			/* Zero out the entire point set. */
			memset (*points, 0, output_numbers_per_point * n * sizeof(double));
		}
		...
		while (list NE NULL) {
			...
			/* Battery - read and store in points array */
			nlp = list;
			list = list -> next;
			b = nlp->mantissa * pow(10.0, nlp->expon);
			free(nlp);  /* match how _gst_read_numlist frees */

			/* Save X,Y,Battery coordinates into array */
			*(p++) = x;
			*(p++) = y;
			*(p++) = b;
		}
...
double			c;
	...
	if (pts NE NULL) {
		...
		for (i = 0; i < npts; i++) {
			double x = *p++;
			double y = *p++;
			double battery = *p++;  /* PSW: skip battery value */
