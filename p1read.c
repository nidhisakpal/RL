...
struct gst_hypergraph *	cip;
	...
	for (i = 0; i < nedges; i++) {
		...
		/* Count the numbers and detach the last one (cost) */
		j = 0;
		prev_hookp = NULL;
		hookp = &line;
		for (;;) {
...
#undef PURE_GRAPH
	...
	if (geometric) {
		...
		for (i = 0; i < nverts; i++) {
			...
			if (version >= GST_PVAL_SAVE_FORMAT_VERSION3) {
				p1->battery = get_dec_double(fp);  /* PSW - V3+ has battery in decimal */
				fprintf(stderr, "DEBUG P1READ: Terminal %d battery=%.2f\n", i, p1->battery);
			} else {
				p1->battery = 0.0;  /* Default for older versions */
				fprintf(stderr, "DEBUG P1READ: Terminal %d default battery=0.0 (version %d)\n", i, version);
			}
		}
	}
	...
	for (i = 0; i < nedges; i++) {
		...
		for (j = 0; j < nt; j++) {
			k = get_d (fp);
			if ((k < 1) OR (k > nverts)) {
				fprintf(stderr, "ERROR: Terminal index k=%d is out of range [1, %d]\n", k, nverts);
				exit(1);
			}
			--k;
			*ip1++ = k;
		...
		if (geometric) {
			...
			for (j = 0; j < ns; j++) {
				...
				if (version >= GST_PVAL_SAVE_FORMAT_VERSION3) {
					...
					p1->battery = 0.0;  /* Steiner points always have battery=0 */
				}
			}
			fsp -> terminals = terms;
			fsp -> steiners = steins;
	...
	if (geometric && cip -> full_trees != NULL) {
